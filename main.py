"""
Symulator sieci bezprzewodowej LTE (łącze w dół, pojedyncza komórka).
Algorytm schedulera: Proportional Fairness (PF)
Metoda symulacji:    Planowanie Zdarzeń (Event-Driven Simulation)

Parametry (zgodne z treścią zadania):
  k = 10         bloków zasobów RB
  l = 3          max RB przydzielanych jednemu UE w jednym slocie
  s = 1 ms       interwał schedulera
  ε = 0.1        prawdopodobieństwo błędu transmisji
  t ~ Exp(λ)     czas między pojawieniami się UE
  d ~ Unif{1..10} × 25 kbit    ilość danych do odebrania
  r_k ~ Unif[20, 800] kbit/s   przepływność na k-tym RB
  τ ~ Exp(1/10 ms)              czas do zmiany warunków propagacyjnych
"""
...
SIM_TIME = 100_000   # ms (100 s – długa symulacja dla stabilnych statystyk)
...
print(f"{'λ [1/ms]':>10} | {'avg_wait [ms]':>14} | {'sys_tp [kbit/ms]':>17} | {'usr_tp [kbit/ms]':>17} | {'ukończeni':>10}")
print("-" * 85)
...
# ── λ* – maksymalna λ spełniająca kryterium ─────────────────────────────────
valid = [r for r in results if r["avg_wait_ms"] <= MAX_WAIT]
if valid:
...
fig.suptitle("Symulator LTE – Proportional Fairness (k=10 RB, l=3, ε=0.1)",
             fontsize=13, fontweight="bold")

# ── Wykres 1: czas oczekiwania vs λ ─────────────────────────────────────────
ax1 = axes[0]
ax1.plot(lam_vals, wait_vals, "o-", color="#01696f", linewidth=2, markersize=7,
         label="avg czas oczekiwania")
ax1.axhline(MAX_WAIT, color="#a12c7b", linestyle="--", linewidth=1.8,
            label=f"Kryterium: {MAX_WAIT} ms")
if valid:
    ax1.axvline(best["lambda"], color="#da7101", linestyle=":", linewidth=1.5,
                label=f"λ* = {best['lambda']:.4f}")
...
# ── Wykres 2: przepływność systemu vs λ ─────────────────────────────────────
ax2 = axes[1]
ln1, = ax2.plot(lam_vals, sys_tp_vals, "s-", color="#006494", linewidth=2,
                markersize=7, label="Przep. systemu [kbit/ms]")
ax2r = ax2.twinx()
ln2, = ax2r.plot(lam_vals, usr_tp_vals, "^--", color="#da7101", linewidth=1.8,
                 markersize=7, label="Przep. użytkownika [kbit/ms]")
...
# ── Histogram przepływności użytkowników dla λ* ──────────────────────────────
if valid and best["user_tps"]:
    ...
    ax3.set_title(
        ...
        f"λ* = {best['lambda']:.4f} 1/ms | PF Scheduler | k=10, l=3", fontsize=11)
